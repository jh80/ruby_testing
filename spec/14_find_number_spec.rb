...
describe FindNumber do
  ...
  describe '#game_over?' do
    let(:goal_number) { double('goal_number', value: 7)}
    context 'when guess and random_number are equal' do
      ...
      subject(:game_done) { described_class.new(3, 9, goal_number, 7)}
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_not_done) { described_class.new(2, 9, goal_number, 8)}

      it 'is not game over' do
        expect(game_not_done.game_over?).to eq(false)
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:last_choice_game) { described_class.new( 5, 8, number_range, 7)}
      
      it 'updates min to the same value as max' do
        expect { last_choice_game.update_range }.to change { last_choice_game.min }.from(5).to(8)
      end

      it 'does not update max' do
        expect { last_choice_game.update_range }.not_to change { last_choice_game.max }
      end
    end
  end
end
